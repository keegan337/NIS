...
//		Create thread to receive messages asynchronously
		...
		networkManager.startAsyncReceiveThread(bytes -> {
			//Decrypt message
			try {
				//TESTING NOTE: try to decrypt the message with a different private key (as an attacker intercepting messages might)
				bytes = CryptoUtils.decryptData(bytes, CertificateUtils.generateKeyPair().getPrivate());
